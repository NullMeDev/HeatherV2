"""
Stripe Payment Intent Gate - Full Transaction Flow
...
def process_payment_intent(card_num, card_mon, card_yer, card_cvc, 
                          amount=5000, currency='usd', proxy=None, timeout=30):
    """
    Process payment using full transaction flow via merchant donation form.
    
    Returns (result_string, proxy_ok_bool)
    """
    fake = Faker()
    ...
    try:
        rpm = session.post('https://api.stripe.com/v1/payment_methods', 
                          headers=headers_pm, data=data_pm, timeout=timeout)
        ...
        try:
            ...
            if not pm_id or not pm_id.startswith('pm_'):
                return ("DECLINED ❌ Invalid Payment Method", True)
...
def stripe_payment_intent_check(card_num, card_mon, card_yer, card_cvc, proxy=None, timeout=30):
    """Alias for process_payment_intent for compatibility."""
    return process_payment_intent(card_num, card_mon, card_yer, card_cvc, 
                                  amount=800, currency='usd', proxy=proxy, timeout=timeout)
