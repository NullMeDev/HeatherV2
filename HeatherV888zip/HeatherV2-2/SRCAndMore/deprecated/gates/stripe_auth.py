...
Uses hardcoded Stripe public key pool for reliability
"""
...
from gates.stripe_multi import STRIPE_PKS

fake = Faker()

_key_rotation_idx = 0

def _get_next_stripe_pk():
    """Rotate through the hardcoded Stripe public key pool for load distribution."""
    global _key_rotation_idx
    pk_info = STRIPE_PKS[_key_rotation_idx % len(STRIPE_PKS)]
    _key_rotation_idx += 1
    return pk_info['pk'], pk_info['name']

# Cache for store keys (deprecated - kept for backwards compatibility)
_key_cache = {}

def _extract_stripe_key_from_page(store_url, session=None):
    """
    DEPRECATED: Extract Stripe publishable key from store page.
    This function is unreliable due to website scraping failures.
    Use _get_next_stripe_pk() instead which uses the hardcoded key pool.
...
def _stripe_auth_check_original(card_num, card_mon, card_yer, card_cvc, store_url=None, proxy=None):
    try:
        ...
        # Step 2: Get Stripe key from hardcoded pool (reliable)
        stripe_key, site_name = _get_next_stripe_pk()
