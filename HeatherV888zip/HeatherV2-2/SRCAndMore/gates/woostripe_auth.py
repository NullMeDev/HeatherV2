...
STRIPE_PKS = [
    {'pk': 'pk_live_51IGU0GIHh0fd2MZ32oi6r6NEUMy1GP19UVxwpXGlx3VagMJJOS0EM4e6moTZ4TUCFdX2HLlqns5dQJEx42rvhlfg003wK95g5r', 'name': 'corrigan'},
    {'pk': 'pk_live_51LTAH3KQqBJAM2n1ywv46dJsjQWht8ckfcm7d15RiE8eIpXWXUvfshCKKsDCyFZG48CY68L9dUTB0UsbDQe32Zn700Qe4vrX0d', 'name': 'texas'},
    {'pk': 'pk_live_51D5KOzFIh1EiUYTpqDjnR7kxYbpLEjNI69EfLVBVwOdmYdmqc7bW3fU3yM0P7w3A1BdGhNMu7mPknFx8LF6tRvBg00G5kBQkVk', 'name': 'fallback1'},
]

_woo_key_rotation_idx = 0

def _get_fallback_stripe_pk():
    """Rotate through the hardcoded Stripe public key pool as fallback."""
    global _woo_key_rotation_idx
    pk_info = STRIPE_PKS[_woo_key_rotation_idx % len(STRIPE_PKS)]
    _woo_key_rotation_idx += 1
    return pk_info['pk'], pk_info['name']
...
def woostripe_auth_check(card_num, card_mon, card_yer, card_cvc, proxy=None, site_url='https://ccfoundationorg.com', timeout=30):
    ...
    if not stripe_pk:
        stripe_pk, fallback_site = _get_fallback_stripe_pk()
