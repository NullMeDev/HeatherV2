"""
Stripe Multi-Site AUTH Gate - Full Authorization Flow
Uses multiple known working Stripe sites with full donation form submission
"""
...
STRIPE_PKS = [{'pk': site['stripe_pk'], 'name': site['name']} for site in MERCHANT_SITES]

_last_used_idx = 0


def _rotate_site() -> dict:
    """Rotate through available sites"""
    global _last_used_idx
    site_info = MERCHANT_SITES[_last_used_idx % len(MERCHANT_SITES)]
    _last_used_idx += 1
    return site_info
