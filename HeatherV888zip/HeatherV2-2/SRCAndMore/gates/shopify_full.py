        ...
        try:
            async with httpx.AsyncClient(
                ...
                if "submitsuccess" in result_text or "receipt" in result_text:
                    ...
                    error_msg = _capture(resp.text, '"nonLocalizedMessage":"', '"')
                    
                    if should_retry_on_response(result_text):
                        raise Exception(f"Retryable: {error_msg or 'Unknown'}")
                    ...
                    if error_msg:
                        ...
                        record_bin_result(card_num, domain, False, "unknown_response")
                        return (f"⚠️ UNKNOWN - Response Unrecognized{bin_display} | {domain}", proxy_alive)
