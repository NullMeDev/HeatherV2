...
SHOPIFY_FILE = os.environ.get("SHOPIFY_FILE", os.path.join(BASE_DIR, "data", "shopify_stores.txt"))
CACHE_FILE = os.environ.get("SHOPIFY_CACHE_FILE", os.path.join(BASE_DIR, ".shopify_cache.json"))
DEAD_STORES_FILE = os.environ.get("SHOPIFY_DEAD_FILE", os.path.join(BASE_DIR, ".shopify_dead.json"))
BAD_STORES_FILE = os.environ.get("SHOPIFY_BAD_FILE", os.path.join(BASE_DIR, ".shopify_bad.json"))
...
def load_shopify_sites(exclude_dead=True):
    """Load all Shopify sites from file, optionally excluding dead stores"""
    try:
        with open(SHOPIFY_FILE, 'r') as f:
            sites = [line.strip() for line in f if line.strip()]
        
        if exclude_dead:
            # dead_stores is now a dict, so check keys
